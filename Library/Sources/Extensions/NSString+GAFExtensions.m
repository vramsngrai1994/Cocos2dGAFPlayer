...
@implementation NSString (GAFxtensions)
...
+ (NSString *)stringFromFileSize:(unsigned long long)fileSize precision:(NSInteger)precision
{
    ...
    NSString *result = [NSString stringWithFormat:format, size, sSufixes[mult]];
            ...
            [NSPredicate predicateWithBlock:^BOOL(id evaluatedObject, NSDictionary *bindings) 
             {
                 ...
                 return (![stringObject isEqual:@""] &&
                         ![[stringObject stringByTrimmingSpaces] isEqual:@""]);
             }]];
}
...
- (BOOL)containsStringAsSubstring:(NSString *)aSubString
{
    if (nil == aSubString || [aSubString length] == 0)
    {
        return YES;
    }
...
- (NSArray *)rangesOfAllWordsInString:(NSString*)aString options:(NSStringCompareOptions)aOptions
{
    ...
    for (NSUInteger i = 0; i < [words count]; ++i)
    {
        ...
        {
            [wordsWithoutDuplicates addObject:words[i]];
        }
    }
